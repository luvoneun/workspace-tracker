...
새 버전이 나오면 `설정 > 상태` 맨 아래에 `새 버전 v1.1.0이 있어요`가 떠요. `업데이트 받기`를 누르면 그 자리에서 진행 목록(백업 → 받기 → 다시 시작 → 확인, 1분쯤)을 보여 주고, 끝나면 `새로고침`을 눌러요. 새 버전을 받기 전에 멈추면 앱과 데이터가 그대로라 `다시 시도`를, 받은 뒤에 실패하면 저절로 되돌리지 않고 `이전 버전으로 되돌리기`를 보여 줘요.
...
`설정 > 꾸미기`(Dock 아이콘·Dock 이름·워크스페이스 제목 — 이 맥에만)와 `local/local.css`, `설정 > 연동`의 값만 지원해요. 코드를 고치면 `설정 > 상태`에 `수정된 파일 N개`로 표시되고, 그 상태는 지원하지 않아요. 좋은 수정은 fork → PR로 보내 주세요.
