...
`설정`(톱니바퀴) → `연동`에서 하나씩 켜요. 설치할 때 아무것도 묻지 않는 이유예요. 켜거나 끄면 앱이 맥 스케줄러(launchd) 등록까지 알아서 해요 — 따로 실행할 것이 없어요(이 기능이 없던 버전에서 올라온 뒤 처음 한 번만 `업데이트.command`).
...
새 버전이 나오면 `설정 > 앱`의 `버전` 줄에 `새 버전 v1.1.0이 있어요`가 떠요(설정을 열지 않아도 톱니바퀴에 파란 점이 붙고, 누르면 앱 탭이 열려요). `업데이트 받기`를 누르면 그 자리에서 진행 목록(백업 → 받기 → 다시 시작 → 확인, 1분쯤)을 보여 주고, 끝나면 `새로고침`을 눌러요. 새 버전을 받기 전에 멈추면 앱과 데이터가 그대로라 `다시 시도`를, 받은 뒤에 실패하면 저절로 되돌리지 않고 `이전 버전으로 되돌리기`를 보여 줘요.
...
**GitHub 비공개 저장소에도 올리고 싶으면**(맥이 고장 나도 남게) 한 겹 더 켤 수 있어요 — `tracker/inbox-app/README.md`의 `업무 데이터 백업` 부분을 따라 한 번 설정하면 같은 19:30 실행이 이어서 올려요. 맥 타임머신도 함께 권해요.
