...
새 버전이 나오면 `설정 > 상태` 맨 아래에 `새 버전 v1.1.0이 있어요`가 떠요. `업데이트 받기`를 누르면 그 자리에서 진행 목록(백업 → 받기 → 다시 시작 → 확인, 1분쯤)을 보여 주고, 끝나면 `새로고침`을 눌러요. 실패하면 저절로 되돌리지 않고 `이전 버전으로 되돌리기`를 보여 줘요.
