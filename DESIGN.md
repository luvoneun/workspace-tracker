...
| 탭 | `오늘 \| 프로젝트 \| 회의 \| 아이디어·결정 \| 주간요약`. 15/500, 선택은 700 + 2.5px 파란 밑줄. 탭 이름 옆 작은 숫자(`d-tabnum`, 12.5px `--dim`)는 0이면 비운다 — `회의`는 검토할 초안이 있는 회의 수, `아이디어·결정`은 PRD 미반영 결정 수 |
...
| ≤900 | 레일·프로젝트 목록·주차 목록이 본문 위로 쌓인다(가로 스크롤 한 줄). 회의 목록도 본문 위로 쌓이되 칩 줄·날짜 소제목이 있어 가로로 흘리지 않고 세로 그대로 두고 높이만 40vh로 줄인다. 서랍은 아래에서 올라오는 전체 폭 시트(최대 86vh, 위쪽 모서리 20px). 아이디어·결정은 1열 |
