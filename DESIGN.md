...
| 회의 탭 | grid `340px minmax(0,1fr)` — 프로젝트 탭과 같은 뼈대이고 왼쪽 카드만 조금 넓다(`시각 · 제목 · 프로젝트 · 상태`가 한 줄에 들어와야 한다). 왼쪽은 흰 카드 목록(토글 칩 `초안 있음`·`미완료만`·`기록 없는 회의도 보기` + 프로젝트 고르는 칸 → 날짜 소제목 → 줄 `42px minmax(0,1fr) auto` → 있으면 `이전 회의 더 보기`), 오른쪽은 **줄 옆 회의 카드와 같은 내용**을 페이지 안의 흰 카드(`.d-detail.is-tab`, 최대 940px)로 그린다 — ✕ 없이, 담기 바는 카드 맨 아래에 `position: sticky` |
...
- **열 순서** 체크박스 | 제목(+ `· ● 프로젝트` + `원문`) | (비어 있는 프로젝트 열) | 상태 열 — 안에서 **우선순위 → 상태(밀림 · N일째 진행 중 · 답변) → 기한(맨 오른쪽)**.
- **줄 태그는 자리가 고정된다**(오늘 목록·나중에 할 일 서랍·`마감·중요도순`, `.d-meta.is-fixed`): `48px minmax(0,1fr) 120px` 격자에 우선순위 | 상태 | 기한 세 칸이 줄마다 같은 x 자리에 서고, **값이 없는 칸도 빈 자리로 남는다** — 같은 종류가 세로로 서야 훑을 수 있다(DECISIONS). 기한 칸만 오른쪽 정렬, 나머지는 칸 왼쪽. 상태가 겹치면(밀림 + 답변) 한 칸에 이어 쓰고 넘치면 말줄임. 완료한 줄·미루기 제안 줄은 예전처럼 있는 만큼만 오른쪽부터 이어 붙인다.
...
- **≤520px** 두 줄 격자 `"ck ti ac" / "ck mt ac"`, 최소 60px. 제목 최대 2줄, 프로젝트는 정보 줄 맨 앞으로, 동작은 항상 보이고 세로로 쌓인다. 자리 고정은 풀고(격자 해제) 같은 순서로 이어 붙인다.

### 상태말 — 말과 아이콘 표

기한·밀림·진행·답변에는 14px 아이콘 + 풀어 쓴 `title` 툴팁이 붙는다. **우선순위는 아이콘 없이 글자와 색뿐이다**(같은 아이콘이 열을 따라 반복되면 산만했다 — DECISIONS). **배지** 칸이 ✓인 것만 틴트 바탕으로 선다.
