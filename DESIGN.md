...
- **확인 대기** — 줄이 두 줄 구성이다: 첫 줄 제목(두 줄까지), 둘째 줄 `결제팀 · 2일째`(3일째부터 `--warn`) + 답변 받을 날이 지났거나 오늘이면 배지 + `9월 24일 다시 확인` / `오늘 요청함`. 다시 확인할 항목은 목록 맨 위로 올라온다. 입력칸은 카드 맨 아래 `+ 확인 대기 추가`.
