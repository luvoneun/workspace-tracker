...
| 회의 탭 | grid `340px minmax(0,1fr)` — 프로젝트 탭과 같은 뼈대이고 왼쪽 카드만 조금 넓다(`시각 · 제목 · 프로젝트 · 상태`가 한 줄에 들어와야 한다). 왼쪽은 흰 카드 목록(토글 칩 `미해결만`·`검토 대기` + 프로젝트 고르는 칸 → 날짜 소제목 → 줄 `42px minmax(0,1fr) auto`), 오른쪽은 **줄 옆 회의 카드와 같은 내용**을 페이지 안의 흰 카드(`.d-detail.is-tab`, 최대 940px)로 그린다 — ✕ 없이, 담기 바는 카드 맨 아래에 `position: sticky` |
...
- **필터 줄** 회색 트랙 위 종류 칩(`전체 할 일 확인 대기 결정 아이디어 회의`) + 트랙 밖 토글 칩(`완료 제외`). 회의 전용 토글(`미해결만`·`검토 대기`)은 회의 탭에 있다. `오늘 신규` 필터가 걸리면 지울 수 있는 칩이 붙는다.
