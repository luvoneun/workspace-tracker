...
| ≤520 | 상세도 아래에서 올라오는 전체 폭 시트가 된다(최대 86vh, 위쪽 모서리 20px, z 47) — 손가락이 닿는 자리. 줄이 두 줄 격자로 쌓이고(최소 60px) hover 동작은 항상 보인다. 팔레트는 전체 폭, 결과는 `56px 1fr` 두 열 최소 46px. 프로젝트 열 이름 줄은 숨긴다 |
...
**회의 정리 카드**(460px)는 같은 부품을 쓴다. 머리는 제목 + `오늘 16:00–17:00 · 프로젝트` + `티로 노트` + ✕, 발은 **담기 바**(초안이 있을 때만): 왼쪽에 `할 일 1 · 확인 대기 1 · 결정 1`, 오른쪽에 1차 `N개 담기`. 결과 카드·초안·`이 회의에서 나온 것`·`이전 회차의 미해결 항목`·`직접 적어 담기`·`기존 항목 연결`은 가운데에서 스크롤한다. 초안 묶음: 문구가 주인공인 회색 채움 입력(`.d-dtxt`) + 종류 세그먼트 + (할 일이면) `나중에 할 일 / 오늘 할 일` 세그먼트 + 날짜 칸, 오른쪽 위에 ✕. 회의 카드 안에서 항목을 열면 같은 카드 안에서 그 항목 상세로 바뀌고 맨 위에 `← 회의로`가 붙는다.
...
- **전체 업무 기록**(세그먼트의 다른 쪽)은 같은 문서 폭 안의 조용한 목록이다. **프로젝트로 묶고**(보고 문서와 같은 프로젝트 차례, 같은 그룹 제목 부품, `프로젝트 없음`은 맨 아래), 줄은 제목 + `원문` \| 오른쪽에 기본값이 아닌 것만(`완료`, `보고에서 제외됨` + 3차 `복원`). `미완료`·`보고에 포함`은 찍지 않는다. 줄을 누르면 업무 상세가 열린다.
