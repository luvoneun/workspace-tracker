...
- **묶기**는 아래 고정 막대를 쓴다 — 일괄 선택 막대와 **같은 부품**(`.d-selbar`): `1개 선택` + `완료한 일 안에서만 고를 수 있어요` + 1차 `선택한 문장 묶기` + `취소`. 묶은 문장 줄의 ⋯에는 `묶음 풀기`가 하나 더 붙는다(서버가 묶기 전 문장을 들고 있는 행에만).
- **묶기·묶음 풀기의 결과**는 알림으로 말한다: `문장 3개를 묶었어요` · `묶음을 풀었어요` + 알림 안의 `되돌리기`(다른 변경은 예전대로 `보고 내용을 저장했어요`). 이 탭에서는 입력칸 밖의 **⌘Z**도 같은 되돌리기이고, 머리줄의 `되돌리기` 버튼은 그대로 남는다 — 되돌릴 길이 셋이어도 부르는 곳은 하나다.
