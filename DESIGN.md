...
프로젝트 표기는 `uiProjectName(item, { short })` 하나가 만든다(지라는 `KEY · 요약`, `short`면 요약만 — 모르면 둘 다 키). 색 점의 키는 표기와 분리돼 있다(`uiProjectColorKey`가 늘 `item.jira`/`group`/`project` 원값을 쓴다) — 짧은 표기가 요약만 보여 줘도 색은 바뀌지 않는다.
