...
가림막(`--scrim`) 위, 위에서 84px, 폭 `min(620px, 100vw - 32px)`, 20px 모서리, `--pop`, 등장 140ms. 회의 모아보기·오늘 신규처럼 **훑어보는 창**(`is-browse`: 종류가 `회의`이거나 `오늘 신규`가 켜져 있을 때, 641px 이상)은 위에서 64px, `min(920px, 100vw - 48px)` × `min(720px, 100dvh - 112px)` 고정 크기로 크게 열고 결과 줄도 54px로 여유를 준다 — 찾는 창과 모아 보는 창은 필요한 크기가 다르다.
