...
Tracking items are organized by type in `tracker/`:
...
3. **In global tracker** - Default to `tracker/[type]s.md` for general items
...
| `due` | `YYYY-MM-DD` | Deadline (tasks only). Always set explicitly by the user — never auto-inferred from ambiguous source text (e.g. Slack "next week") |
| `source` | `slack:permalink` | Origin link when the item came from a Slack thread |
...
### Task with deadline (from Slack)
```markdown
- 금주 회의록 정리해서 공유 #task[id:task_01HQXYZ7890ABCDEF12349 status:to-do priority:medium created:2026-09-15 due:2026-09-19 source:slack:https://gna.slack.com/archives/C123/p1234567890]
```
...
4. **Add to appropriate tracker file** in `tracker/[type]s.md`
