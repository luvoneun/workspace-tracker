...
구글 캘린더에서 오늘 일정을 가져와 `tracker/calendar_today.md`에 저장합니다. 인박스 앱 왼쪽 상단 "오늘 일정" 카드가 이 파일을 읽어요.
...
일정이 없으면 헤더만 남기고 항목 줄은 비운다. 한 줄에 `- HH:MM-HH:MM | 제목` 형식을 정확히 지킬 것 (앱 파서가 이 형식에 고정돼 있음).
